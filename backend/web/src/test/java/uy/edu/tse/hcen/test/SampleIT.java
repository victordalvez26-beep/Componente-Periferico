...
public class SampleIT {
    ...
    EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "nodo-periferico-ear.ear");
    ...
    File f = new File("../ejb/target/nodo-periferico-ejb.jar");
    JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "nodo-periferico-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/nodo-periferico-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "nodo-periferico-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
